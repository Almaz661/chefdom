...
// Также покрывает мусорные хвосты через табуляцию/пробелы: «1,49 € ca co», «0,79 B B», «2,39 € /»
const PRICE_ONLY_RE = /^-?\d{1,4}[.,]\d{2}\s*(?:€|EUR)?\s*(?:[\w€/.*]{0,4}[\s\t]*)*$/;
...
// Достаём само число из строки-цены (вырезав €, EUR, BTW-коды и OCR-мусор).
// Мусорный хвост — это произвольные короткие токены после числа:
// «€ B», «ca co», «0O 00», «€ /», «B B» и т.д.
function priceOnlyValue(line: string): number | null {
  const cleaned = line
    .replace(/€|EUR/gi, ' ')
    // Удаляем все короткие (1-4 символа) токены ПОСЛЕ цифровой части
    .replace(/(\d[.,]\d{2})\s+.*$/, '$1')
    .trim();
...
function parseAll(text: string, total: number | null): ItemCandidate[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (looksLikeProductName(line)) {
      ...
      // После цены может идти мусор от OCR: «€ B», «ca co», «0O 00», «€ /», «B B»
      // Обрезаем всё после цены — хвост из коротких токенов (≤4 символов каждый)
      const tailMatch = line.match(/(.+?)\s+(-?\d{1,4}[.,]\d{2})(?:\s+[\w€/*.,]{1,4})*\s*$/);
      if (tailMatch) {
