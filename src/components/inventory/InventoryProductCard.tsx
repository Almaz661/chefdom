    ...
    <div className="flex items-center gap-3 rounded-xl px-4 py-3 border border-[var(--cd-line)] bg-white/[0.02] hover:bg-white/[0.04] hover:border-[var(--cd-line)] transition-all duration-200 group">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-base font-semibold text-white/90 truncate">
          {item.isBasic && (
            <span className="text-[var(--cd-gold)]/70 mr-1 text-xs" title="Базовый продукт — не попадает в покупки">📌</span>
          )}
          ...
          {item.minQuantity && (
            <span className="text-[var(--cd-gold)]/60 ml-1.5 text-xs" title="Мин. остаток для авто-докупки">
      ...
      {item.source === 'inventory' && onToggleBasic && (
        <button
          ...
          className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all duration-200 shrink-0 ${
            item.isBasic
              ? 'text-[var(--cd-gold)] bg-[var(--cd-gold)]/10 border border-[var(--cd-gold)]/20'
              : 'text-white/20 hover:text-[var(--cd-gold)] hover:bg-[var(--cd-gold)]/5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100'
          }`}
