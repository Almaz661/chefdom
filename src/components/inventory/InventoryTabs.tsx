...
export function InventoryTabs({
  ...
  return (
    <div className="flex gap-1 p-1 rounded-2xl border border-[var(--cd-line)] bg-[var(--cd-paper)]/60 shrink-0">
      {TABS.map(({ key, label, icon: Icon }) => (
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-bold transition-all duration-200 ${
            active === key
              ? 'bg-gradient-to-r from-[#c9a84c]/20 to-[#c9a84c]/10 text-[var(--cd-gold)] border border-[var(--cd-gold)]/30 shadow-[0_0_12px_rgba(201,149,60,0.15)]'
