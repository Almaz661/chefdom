  ...
  return (
    ...
    <div className="max-w-2xl mx-auto px-5 py-8 lg:py-12">
      <div className="flex items-center justify-between mb-8">
        <h1 className="font-serif text-3xl font-semibold text-white/80">
          Заготовки
        </h1>
        <button
          onClick={() => setShowAdd(true)}
          className="w-9 h-9 rounded-lg bg-[var(--cd-gold)] text-[#0a0c10] flex items-center justify-center hover:bg-[var(--cd-gold-dark)] transition-colors"
        ...
        {TABS.map(({ key, shortLabel, icon: Icon }) => (
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-md text-base font-semibold transition-colors ${
              tab === key
                ? "bg-[var(--cd-gold)] text-[#0a0c10] font-bold"
      ...
      {isLoading ? (
        ...
        <>
                    ...
                    >
                      ...
                      {(tab === "cooked" || tab === "frozen") && item.servings && item.servings > 0 && (
                        <button
                          ...
                          className="flex items-center gap-1 px-2.5 h-8 text-sm font-bold text-[var(--cd-gold)] hover:bg-[var(--cd-gold)]/10 transition-colors shrink-0 border border-[var(--cd-gold)]/40 rounded-lg"
          ...
          {/* Основной список */}
          {items.length === 0 ? (
            <div className="bg-white/[0.03] border border-[var(--cd-line)] border-dashed rounded-[var(--cd-r-xl)] p-8 text-center">
              <TabIconComp
                size={32}
            ...
            <ul className="space-y-1">
              {normal.map((item) => (
                <li
                  key={item.id}
                  className="flex items-center gap-3 bg-white/[0.03] rounded-lg px-4 py-3 border border-[var(--cd-line)]"
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-base font-semibold text-white/80 truncate">
                      {item.name}
...
function AddPreserveDialog({
  ...
  return (
    ...
    >
      <div
        className="bg-[var(--cd-paper)] border border-[var(--cd-line)] rounded-[var(--cd-r-xl)] w-full sm:max-w-sm sm:rounded-[var(--cd-r-xl)] rounded-t-[var(--cd-r-xl)] p-6"
        onClick={(e) => e.stopPropagation()}
      >
        <h3 className="font-serif text-lg font-semibold text-white/80 mb-1 inline-flex items-center gap-2">
          <TitleIcon size={20} className="text-[var(--cd-gold)]" />
        ...
        <form onSubmit={handleSubmit} className="space-y-3 mt-4">
          <input
            ...
            autoFocus
            required
            className="w-full h-12 px-4 bg-white/[0.04] border border-[var(--cd-line)] rounded-xl text-white/80 placeholder-white/25 focus:outline-none focus:border-[var(--cd-gold)]/50"
          />
          <div className="flex gap-2">
            <input
              ...
              step="any"
              min="0"
              className="flex-1 h-12 px-4 bg-white/[0.04] border border-[var(--cd-line)] rounded-xl text-white/80 placeholder-white/25 focus:outline-none focus:border-[var(--cd-gold)]/50"
            />
            <input
              type="text"
              value={unit}
              ...
              className="w-28 h-12 px-4 bg-white/[0.04] border border-[var(--cd-line)] rounded-xl text-white/80 placeholder-white/25 focus:outline-none focus:border-[var(--cd-gold)]/50"
            />
          </div>
          ...
          <label className="block">
            ...
            <input
              ...
              className="w-full h-12 px-4 bg-white/[0.04] border border-[var(--cd-line)] rounded-xl text-white/80 placeholder-white/25 focus:outline-none focus:border-[var(--cd-gold)]/50 [color-scheme:dark]"
            />
          </label>
          <label className="block">
            <span className="block text-base text-white/60 font-medium-soft font-medium mb-1">
              ...
              <span className="mt-1.5 inline-flex items-center gap-1 text-xs text-green-400">
                ...
                {expiryDirty && (
                  <button
                    ...
                    className="ml-1 text-[var(--cd-gold)] underline"
                  >
                    применить
                  </button>
                )}
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 h-12 rounded-xl bg-white/[0.04] border border-[var(--cd-line)] text-white/60 font-medium hover:border-white/[0.15] hover:text-white/80 transition-colors"
            >
              Отмена
            </button>
            <button
              ...
              className="flex-1 h-12 rounded-xl bg-[var(--cd-gold)] text-[#0a0c10] font-semibold hover:bg-[var(--cd-gold-dark)] disabled:opacity-50 transition-colors"
            >
