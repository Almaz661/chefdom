...
export function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[var(--cd-bg)]">
          ...
          <div className="mt-8 space-y-1">
            {expiringTotal > 0 && (
              ...
              >
                ...
                <ArrowRight size={12} className="text-white/40 group-hover:text-[var(--cd-gold)] transition-colors" />
              </Link>
            )}
            {belowMin.length > 0 && (
              <Link
        ...
        {todayMeal ? (
          ...
          >
            <div className="aspect-[16/7] bg-white/[0.03]">
              {todayMeal.recipe.imageUrl ? (
                <img
                  ...
                  className="w-full h-full object-cover photo-cinematic group-hover:brightness-110 transition-transform duration-700"
            ...
            <div className="absolute bottom-0 inset-x-0 p-6 lg:p-8">
              <p className="text-[var(--cd-gold)]/90 text-sm font-bold uppercase tracking-[0.25em] mb-2">
          ...
          <Link
            to="/menu"
            className="block rounded-2xl border border-dashed border-[var(--cd-line)] py-16 lg:py-20 text-center group hover:border-[var(--cd-gold)]/20 transition-colors"
          >
      ...
      <div className="max-w-3xl mx-auto px-6 pb-12">
        <div className="grid grid-cols-3 gap-6">
          <Link to="/what-to-cook" className="text-center group">
            <div className="w-12 h-12 mx-auto rounded-full bg-white/[0.03] border border-[var(--cd-line)] flex items-center justify-center group-hover:border-[var(--cd-gold)]/40 transition-colors mb-3">
              <ChefHat size={18} className="text-[var(--cd-gold)]" strokeWidth={1.5} />
            </div>
          ...
          <Link to="/shopping" className="text-center group">
            <div className="w-12 h-12 mx-auto rounded-full bg-white/[0.03] border border-[var(--cd-line)] flex items-center justify-center group-hover:border-[var(--cd-gold)]/40 transition-colors mb-3 relative">
              <ShoppingCart size={18} className="text-[var(--cd-gold)]" strokeWidth={1.5} />
              {shoppingCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-[var(--cd-gold)] text-[9px] font-bold text-[#0a0c10] flex items-center justify-center">
          ...
          <Link to="/preserves" className="text-center group">
            <div className="w-12 h-12 mx-auto rounded-full bg-white/[0.03] border border-[var(--cd-line)] flex items-center justify-center group-hover:border-[var(--cd-gold)]/40 transition-colors mb-3">
              <Snowflake size={18} className="text-[var(--cd-gold)]" strokeWidth={1.5} />
            </div>
      ...
      <div className="max-w-3xl mx-auto px-6 pb-12 space-y-10">
        ...
        {topRecipe && topRecipe.count >= 2 && (
          <div className="flex items-center gap-4">
            <div className="w-9 h-9 rounded-full bg-white/[0.03] border border-[var(--cd-line)] flex items-center justify-center shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              {topRecipe.recipeId ? (
                <Link to={`/recipes/${topRecipe.recipeId}`} className="text-base text-white/80 font-medium hover:text-[var(--cd-gold)] transition-colors truncate block">
        ...
        {recentCooks.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/history" className="text-xs text-[var(--cd-gold)] uppercase tracking-wider hover:text-[var(--cd-gold-dark)] transition-colors">
                Всё
              </Link>
            </div>
      ...
      <div className="max-w-3xl mx-auto px-6 pb-16">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/menu" className="text-xs text-[var(--cd-gold)] uppercase tracking-wider hover:text-[var(--cd-gold-dark)] transition-colors">
            Меню
          </Link>
        </div>
            ...
            return (
              <Link to="/menu" key={label} className="flex flex-col items-center gap-2 group">
                <span className={`text-[9px] uppercase tracking-wider transition-colors ${
                  isToday ? "text-[var(--cd-gold)]" : "text-white/40 group-hover:text-white/60"
                }`}>
                  {label}
                </span>
                <span className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                  isToday
                    ? "bg-[var(--cd-gold)] text-[#0a0c10]"
                    : filled
                      ? "border border-[var(--cd-gold)]/30 text-[var(--cd-gold)]"
                      : "border border-[var(--cd-line)] text-white/40 group-hover:border-white/[0.15]"
                }`}>
